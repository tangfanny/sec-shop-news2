    ...
    public function lists(){
        ...
        switch ($type) {
            ...
            case '1':
                ...
                $data['rows'] = $this->day_rank->alias('d')->field($field)->join($join)->where($sqlmap)->where($t)->limit($page)->select();
//                    var_dump($this->day_rank->alias('d')->field($field)->join($join)->where($sqlmap)->where($t)->limit($page)->_sql());exit;
            ...
            case '2': //周
                ...
                $data['rows'] = $this->week_rank->where($sqlmap)->where($t)->limit($page)->select();
                $info = $this->week_rank->lastweek_data();
                break;
            case '3': //月
                $t= $this->month_rank->month_time();
                $data['total'] = $this->month_rank->where($t)->count();
                $data['rows'] = $this->month_rank->where($sqlmap)->where($t)->limit($page)->select();
                $info = $this->month_rank->lastmonth_data();
                break;
            case '4': //全部
                $data['total'] = $this->rank_sum->count();
                $data['rows'] = $this->rank_sum->where($sqlmap)->limit($page)->select();
                break;
        }
    ...
    public function edit(){
        ...
        $where1['bid'] =array(eq,$name['bid']) ;
        $day_data = $this->day_rank->get_day_data($where1);
        $edit_data = $this->rank_edit->get_edit_data($where);
//        var_dump($this->day_rank->alias('day')->join($join)->field($field)->where($sqlmap)->_sql());exit();
        if(IS_POST ){
